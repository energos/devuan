...
#+begin_src bash
  apt install openssh-server openssh-client gawk mawk- locales man-db manpages manpages-dev pciutils file traceroute mime-support lsof curl wget plocate nmap ncat bc psmisc git silversearcher-ag build-essential fortune-mod fortunes fortunes-debian-hints flex bison aptitude debootstrap gpm chrony ncal evtest ripgrep htop ckermit tio deborphan imagemagick rpm sqlite3 lsb-release zstd unzip
#+end_src
